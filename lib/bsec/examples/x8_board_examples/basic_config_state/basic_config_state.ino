...
/**
 * @brief : This function checks the BSEC status, prints the respective error code. Halts in case of
 * error
...
void setup(void)
{
	...
	bsecSensor sensorList[] = {BSEC_OUTPUT_RAW_TEMPERATURE, BSEC_OUTPUT_RAW_PRESSURE,
				   BSEC_OUTPUT_RAW_HUMIDITY,	BSEC_OUTPUT_RAW_GAS,
				   BSEC_OUTPUT_RAW_GAS_INDEX,	BSEC_OUTPUT_GAS_ESTIMATE_1,
				   BSEC_OUTPUT_GAS_ESTIMATE_2,	BSEC_OUTPUT_GAS_ESTIMATE_3,
	...
	for (uint8_t i = 0; i < NUM_OF_SENS; i++) {
		...
		if (!envSensor[i].begin(BME68X_SPI_INTF, commMuxRead, commMuxWrite, commMuxDelay,
					&commConfig[i])) {
			checkBsecStatus(envSensor[i]);
		}

		/* Load the configuration string that stores information on how to classify the
		 * detected gas */
		...
		if (!envSensor[i].updateSubscription(sensorList, ARRAY_LEN(sensorList),
	...
	Serial.println("\nBSEC library version " + String(envSensor[0].version.major) + "." +
...
void newDataCallback(const bme68xData data, const bsecOutputs outputs, Bsec2 bsec)
{
	...
	for (uint8_t i = 0; i < outputs.nOutputs; i++) {
		...
		case BSEC_OUTPUT_GAS_ESTIMATE_4:
			if ((int)(output.signal * 10000.0f) >
			    0) /* Ensure that there is a valid value xx.xx% */
			{
				Serial.println("\t" +
					       gasName[(int)(output.sensor_id -
							     BSEC_OUTPUT_GAS_ESTIMATE_1)] +
					       String(" probability : ") +
					       String(output.signal * 100) + "%");
